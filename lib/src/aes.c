...
#define isb_data(w) {   /* inverse S Box */                    \
...
{
	int outlen = compute_out_length( inlen );
	Zeropadding( in, out, inlen, outlen );
...
uint8_t aes_ecb_encrypt(const unsigned char *in, unsigned char *out, int len, const aes_context ctx[1]) {
  ...
  while(i < len) {
    block_copy(tmp, in);
    if (aes_encrypt(tmp, tmp, ctx) != EXIT_SUCCESS)
    {
      return EXIT_FAILURE;
    }
    block_copy(out, tmp);
...
int aes_ecb_decrypt( const uint8_t *in, uint8_t *out, unsigned int inlen, const aes_context ctx[1] )
{
	...
	while ( i < inlen )
	{
		if ( aes_block_decrypt( in, out, ctx ) != 0 )
		{
			return(-1);
		}
		in	+= N_BLOCK;
...
int aes_ecb_pkcs7_decrypt( const uint8_t *in, uint8_t *out, unsigned int inlen, const aes_context ctx[1] )
{
	return(aes_ecb_decrypt( in, out, inlen, ctx ) );
}
